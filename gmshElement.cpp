...
void Element::changeNodes(const std::vector<size_t>& nodes) {_nodes = nodes;}
void Element::changeNode(const size_t position, const size_t node) {_nodes[position] = node;}
void Element::changeIndex(const size_t index) {_index = index;}
void Element::changeTypeElem(const size_t type_elem) {_type_elem = type_elem;}
void Element::changePhysicalEntities(const size_t physical_entities) {_physical_entities = physical_entities;}
void Element::changeElemTag(const size_t elem_tag) {_elem_tag = elem_tag;}
