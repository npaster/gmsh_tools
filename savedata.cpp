...
Data::Data(const int nb_composante, const std::vector< std::pair<int, std::vector<double> > >& data)
            : _nb_composante(nb_composante), _data(data) {}

int Data::getNbComposante() const
{
   return _nb_composante;
}

std::vector< std::pair<int, std::vector<double> > > Data::getData() const
{
   return _data;
}

void Data::addData(const int num, const std::vector<double>& values)
{
   _data.push_back(std::make_pair(num, values));
}


void Data::modifyNbComposante(const int nb_composante)
{
...
void NodeData::writeNodeData(const std::string name_mesh) const
{
   ...
   const std::vector< std::pair<int, std::vector<double> > > result=  _data.getData();
